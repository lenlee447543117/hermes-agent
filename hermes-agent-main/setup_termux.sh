...
log_info "========================================="
log_info "  AI沪老 Termux Edge Agent 一键初始化"
log_info "========================================="

# 1. 更新 Termux 包
log_info "Step 1/8: 更新 Termux 包管理器..."
pkg update -y && pkg upgrade -y

# 2. 安装基础依赖
log_info "Step 2/8: 安装基础依赖..."
pkg install -y python termux-api android-tools termux-services

# 3. 安装 Python 依赖
log_info "Step 3/8: 安装 Python 依赖..."
pip install --upgrade pip
pip install uiautomator2 websocket-client requests fastapi uvicorn vosk opencv-python-headless

# 4. 下载 Vosk 离线中文模型
log_info "Step 4/8: 下载 Vosk 离线中文模型（小模型约40MB）..."
...
# 6. 初始化 uiautomator2 ATX 守护
log_info "Step 6/8: 初始化 uiautomator2 ATX 守护..."
python -c "import uiautomator2 as u2; u2.connect()" 2>/dev/null || log_warn "uiautomator2 初始化失败，请确认 ADB 连接正常"

# 7. 获取 agent_core.py
log_info "Step 7/8: 获取 HulaoEdgeAgent 代码..."
AGENT_DIR="$HOME/hermes-agent-main"
if [ -d "$AGENT_DIR" ]; then
    cd "$AGENT_DIR"
    git pull 2>/dev/null || log_warn "Git pull 失败，使用本地代码"
else
    log_warn "未找到 $AGENT_DIR，请手动将 agent_core.py 放置到 $AGENT_DIR/"
    mkdir -p "$AGENT_DIR"
fi

# 8. 注册为 Termux 服务（自启动）
log_info "Step 8/8: 注册为 Termux 服务..."
SVCS_DIR="$HOME/.termux/svcs"
mkdir -p "$SVCS_DIR"

cat > "$SVCS_DIR/hulao-agent" << 'EOF'
#!/data/data/com.termux/files/usr/bin/bash
cd ~/hermes-agent-main
exec uvicorn agent_core:app --host 127.0.0.1 --port 8000
EOF
chmod +x "$SVCS_DIR/hulao-agent"

mkdir -p "$SVCS_DIR/hulao-agent/log"
cat > "$SVCS_DIR/hulao-agent/run" << 'EOF'
#!/data/data/com.termux/files/usr/bin/bash
cd ~/hermes-agent-main
exec uvicorn agent_core:app --host 127.0.0.1 --port 8000 2>&1
EOF
...
echo ""
log_info "启动方式："
echo "  cd ~/hermes-agent-main && uvicorn agent_core:app --host 127.0.0.1 --port 8000"
echo ""
log_info "服务自启动："
